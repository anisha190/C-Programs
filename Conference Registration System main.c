...
void displayParticipants(struct Participant participants[], int count) {
    ...
    printf("Name: %s", participants[x].name);
    printf("ID: %d", participants[x].id);
    printf("Organization: %s", participants[x].organization);
    printf("Registration fees: %.3f", participants[x].fees);
    printf("Payment status: %s", y);
	}
}
